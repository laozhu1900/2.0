...
public class ArticleDaoImpl extends HibernateDaoSupport implements ArticleDao ,HibernateCallback{
    ...
    public void update(final String title, final String author,final String context, final long editTime, final int id) {

        this.getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {

                Query query = session.createQuery("update Article a set a.title=?,a.author=?,a.context=?,a.editTime=? where id=?");

                query.setString(0,title);
                query.setString(1,author);
                query.setString(2,context);
                query.setLong(3,editTime);
                query.setInteger(4,new Integer(id));
